...
void FCFS(char InputFile[40], char OutputFile[40]){
    ...
    int calculate_no_days(int *plant_produced, int *days_used, char orders[100][4][11], char Plant_X[30][4][11], char Plant_Y[30][4][11], char Plant_Z[30][4][11], int i )
    {
        ...
        char Starting[] = strcpy(Starting, startdate);
        while(quantity != 0)
        {
            if(quantity <= 700 || quantity >= 501)
            {
                ...
                plant_produced[1] += 1;
                days_used[1] += 1;
                updateY(quantity, i, Starting );


                quantity -= X_CAPACITY;
                plant_produced[0] += 1;
                days_used[0] += 1;
                updateX(quantity, i, Starting );

            }
            else if(quantity >= 500 || quantity > 400){
            
                quantity -= Z_CAPACITY;
                plant_produced[2] += 1;
                days_used[2] += 1;
                updateZ(quantity, i, Starting );
