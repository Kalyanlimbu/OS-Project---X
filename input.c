...
int substring(char str[150], char substr[150]) {
    int startsWith = 0;
    int i = 0;
...
int main(){
    ...
    while (1){
        printf ("\nPlease Enter:\n> ");
        char input[150];
        scanf ("%s", &input); // reading input
        // if condition
        if (substring(input, command1)==1){
            // update the period
            char delim[] = " ";
            ...
            while((ptr != NULL)) {
                ...
                printf("%s\n", temp);
                strcpy(dates[datenum], temp);
                datenum++;
            }
            // setting the period
            strcpy(startdate, dates[1]);
            strcpy(enddate, dates[2]);
            printf ("The StartDate is: %s\n", startdate);
            printf ("The EndDate is: %s\n", enddate);
        }
        else if (substring(input, command2)==1){
            // update orders
            char delim[] = " ";
            char *ptr = strtok(input, delim);
            char temp[11];
            ...
            while((ptr != NULL)) {
                sprintf(temp, "%s", ptr);
                ptr = strtok(NULL, delim);
                strcpy(orders[orderno][descPointer], temp);
                descPointer++;
            }
            orderno++;
        }
        else if (substring(input, command3)==1){
            // read the file
        }
        else if (substring(input, command4)==1){
            // running the algorithm
        }
        else if (substring(input, command6)==1){
            // exit
